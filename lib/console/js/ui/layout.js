...
DygraphCanvasRenderer.prototype.render = function () {
    ...
    if (this.options.underlayCallback) {
        this.options.underlayCallback(b, this.area, this.layout)
    }
...
Dygraph.prototype.createInterface_ = function () {
    ...
    var c = {top: 0, left: this.attr_("yAxisLabelWidth") + 2 * this.attr_("axisTickSize")};
...
Dygraph.prototype.mouseMove_ = function (b) {
    ...
    if (this.attr_("highlightCallback")) {
        var n = this.lastHighlightCallbackX;
        if (n !== null && m != n) {
            this.lastHighlightCallbackX = m;
...
Dygraph.prototype.hmsString_ = function (a) {
...
Dygraph.prototype.addXTicks_ = function () {
    ...
    this.layout_.updateOptions({xTicks: b})
};
Dygraph.SECONDLY = 0;
...
Dygraph.prototype.GetXAxis = function (n, k, a) {
    var y = [];
    if (a < Dygraph.MONTHLY) {
        var e = Dygraph.SHORT_SPACINGS[a];
        var u = "%d%b";
        var v = e / 1000;
        var w = new Date(n);
        if (v <= 60) {
        ...
        for (var l = n; l <= k; l += e) {
            var w = new Date(l);
            var b = w.getHours() * 3600 + w.getMinutes() * 60 + w.getSeconds();
            if (b == 0 || a >= Dygraph.DAILY) {
                y.push({v: l, label: new Date(l + 3600 * 1000).strftime(u)})
            } else {
                y.push({v: l, label: this.hmsString_(l)})
            }
        }
    } else {
        var f;
        ...
        var r = new Date(n).getFullYear();
        var p = new Date(k).getFullYear();
        var c = Dygraph.zeropad;
        for (var s = r; s <= p; s++) {
            if (s % o != 0) {
                continue
            ...
            for (var q = 0; q < f.length; q++) {
                ...
                y.push({v: l, label: new Date(l).strftime("%b %y")})
            }
        }
    }
    return y
...
Dygraph.prototype.addYTicks_ = function (c, b) {
    ...
    this.layout_.updateOptions({yAxis: [c, b], yTicks: a})
};
